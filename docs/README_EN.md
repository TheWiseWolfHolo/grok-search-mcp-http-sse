...
| `GROK_SEARCH_INCLUDE_QUERY_DIAGNOSTIC` | ❌ | `true` | Include `incoming_query` and `effective_query` diagnostic lines in `web_search` output |
...
- `web_search` prefixes output with a model info line (`search_model/judge_model/retry/temporal_verdict/correction`) by default for easier debugging
- `web_search` also includes `incoming_query` and `effective_query` lines by default (`GROK_SEARCH_INCLUDE_QUERY_DIAGNOSTIC=true`)
